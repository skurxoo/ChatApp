...
function spawnEnemy(){const spots=MAPS[mapIndex].spawns,spawn=spots[enemies.length%spots.length];enemies.push({x:spawn[0],y:spawn[1],vx:0,vy:0,face:spawn[0]<player.x?1:-1,health:35+level*18,hurt:0,attack:0,hit:-1,name:['Nora','Elias','Max','Luna','Otto','Ivy'][enemies.length%6]})}
...
function update(dt){
 ...
 player.hurt=Math.max(0,player.hurt-dt);camera=0;
  ...
  // Update tiger position to follow player
  if(!room && tiger.active){
    const targetX = player.x - 100;
    const targetY = player.y;
    const speed = 0.1;
    tiger.x += (targetX - tiger.x) * speed;
    tiger.y += (targetY - tiger.y) * speed;
    
    // Update tiger face direction based on movement
    if(player.vx > 5) tiger.face = 1;
    else if(player.vx < -5) tiger.face = -1;
  }
 
  // Enemy spawning logic for solo mode
  if(!room) {
    enemySpawnClock -= dt;
    if(enemySpawnClock <= 0 && enemies.length < Math.min(3 + level, 6) && kills < target) {
      spawnEnemy();
      enemySpawnClock = 2; // Spawn every 2 seconds
    }
  }
 
 spawnClock-=dt;if(!room&&spawnClock<=0&&enemies.length<Math.min(3+level,6)&&kills<target){spawnEnemy();spawnClock=1.5+Math.random()}
  for(const e of enemies){const dx=player.x-e.x,d=Math.abs(dx);e.face=Math.sign(dx)||e.face;e.hurt=Math.max(0,e.hurt-dt);e.attack=Math.max(0,e.attack-dt);if(e.hurt<=0&&d>28)e.vx+=Math.sign(dx)*300*dt;e.vx*=Math.pow(.015,dt);e.vx=Math.max(-78-level*5,Math.min(78+level*5,e.vx));e.x+=e.vx*dt;if(d<34&&e.attack<=0)e.attack=.8;if(e.attack>0)e.attack-=dt;if(e.attack<=0&&player.hurt<=0)if(e.attack>.38&&e.attack<.46&&player.hurt<=0)damagePlayer(1+level/4,Math.sign(dx));if(player.sword&&e.hit!==attackId&&swordHits(e.x,e.y)){e.hit=attackId;e.health-=34;e.hurt=.25;e.vx=player.face*150;puff(e.x,e.y,8)}}
 if(player.sword){for(const o of objects)if(!o.dead&&swordHits(o.x,o.y-o.h*.45,26)){const key='o'+o.id;if(!player.hit.has(key)){player.hit.add(key);o.hp--;puff(o.x,o.y-o.h/2,18);if(o.hp<=0){o.dead=true;if(o.kind==='barrel'||o.kind==='explosive')explodeBarrel(o)}}}for(const [name,r]of remote)if(r.health>0&&swordHits(r.x,r.y,14)){const key='r'+name;if(!player.hit.has(key)){player.hit.add(key);send(`HIT|${enc(name)}|18`)}}}else player.hit.clear();
...
function swordHit(x,y){for(let i=0;i<24;i++)bits.push({type:'spark',x,y,vx:(Math.random()-.5)*200,vy:-Math.random()*180,life:.2+Math.random()*.4,size:1+Math.random()*2,color:Math.random()>.5?'#ffffff':'#ffd700'})}
...
function drawCat(x,y,face,attack,hurt,health,name){
   ...
   // Removed the title/label that appears above the tiger in solo mode
   // The health bar and name tag drawing is kept for visual consistency
   ctx.fillStyle='#430b0b';
   ctx.fillRect(-25,-44,50,5);
   ctx.fillStyle=health>35?'#75e07a':'#ff554f';
   ctx.fillRect(-25,-44,50*Math.max(0,health)/100,5);
...
function connect(command){if(socket)socket.close();myName=$('#playerName').value.trim().slice(0,24)||'Swordsman';localStorage.swordsName=myName;const proto=location.protocol==='https:'?'wss:':'ws:';socket=new WebSocket(`${proto}//${location.host}/swords`);ui.status.textContent='Connecting…';socket.onopen=()=>send(`${command}|${enc(myName)}${command==='JOIN'?'|'+$('#roomCode').value.toUpperCase():''}`);socket.onmessage=e=>{const p=e.data.split('|');if(p[0]==='ROOM'){room=p[1];ui.status.textContent=`Room ${room} · waiting for fighters`;startGame()}else if(p[0]==='PLAYERS'){playerCount=p.slice(1).filter(Boolean).length;ui.status.textContent=playerCount>1?`${playerCount}/6 players · last fighter standing wins`:`${playerCount}/6 player · waiting for another fighter`;updateHud()}else if(p[0]==='STATE'){const n=dec(p[1]);if(n!==myName)remote.set(n,{x:+p[2],y:+p[3],face:+p[4],angle:+p[5],health:+p[6],map:+p[7],level:+p[8]})}else if(p[0]==='DAMAGE'){player.health=Math.max(0,+p[1]);player.hurt=.5;if(player.health<=0){running=false;controls.moveX=0;controls.sword=false}}else if(p[0]==='ROUND'){roundWinner=dec(p[1]);roundEndAt=Date.now()+(+p[2]||5)*1000;running=false;ui.round.querySelector('strong').textContent=roundWinner?`${roundWinner} WINS THE ROUND`:'ROUND DRAW';ui.round.classList.remove('hidden')}else if(p[0]==='RESET'){roundNumber=+p[1]||roundNumber+1;roundEndAt=0;ui.round.classList.add('hidden');reset();running=true}else if(p[0]==='LEFT')remote.delete(dec(p[1]));else if(p[0]==='ERROR')ui.status.textContent=p.slice(1).join(' ')};socket.onerror=()=>ui.status.textContent='Could not connect to multiplayer server';socket.onclose=()=>{if(room)ui.status.textContent='Multiplayer disconnected';room='';updateHud()}}
