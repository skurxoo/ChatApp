...
function update(dt){
 ...
 spawnClock-=dt;if(!room&&spawnClock<=0&&enemies.length<Math.min(3+level,6)&&kills<target){spawnEnemy();spawnClock=2}
  for(const e of enemies){const dx=player.x-e.x,d=Math.abs(dx);e.face=Math.sign(dx)||e.face;e.hurt=Math.max(0,e.hurt-dt);e.attack=Math.max(0,e.attack-dt);if(e.hurt<=0&&d>28)e.vx+=Math.sign(dx)*300*dt;e.vx*=Math.pow(.015,dt);e.vx=Math.max(-78-level*5,Math.min(78+level*5,e.vx));e.x+=e.vx*dt;const eoy=e.y;e.vy+=800*dt;e.y+=e.vy*dt;e.grounded=false;let efloor=0;for(const p of platforms())if(e.x>p.x-10&&e.x<p.x+p.w+10&&eoy<=p.y&&e.y>=p.y&&e.vy>=0)efloor=Math.min(efloor,p.y);if(e.y>=efloor){e.y=efloor;e.vy=0;e.grounded=true}if(d<34&&e.attack<=0)e.attack=.8;if(e.attack>.38&&e.attack<.46&&player.hurt<=0)damagePlayer(1+level/4,Math.sign(dx));if(player.sword&&e.hit!==attackId&&swordHits(e.x,e.y)){e.hit=attackId;e.health-=34;e.hurt=.25;e.vx=player.face*150;e.vy=-120;puff(e.x,e.y,8)}}
